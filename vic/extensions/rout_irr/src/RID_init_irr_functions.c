...
void alloc_init_set_irrigation() {
    ...
    size_t iCrop;
             ...
             *******************************/
            ivar_global_veg = malloc(global_param.nr_crops * sizeof (*ivar_global_veg));
    ...
    for (j = 0; j < global_param.nr_crops; j++) {
        for (i = 0; i < local_domain.ncells_active; i++) {
            ivar_local_veg[i] = veg_con_map[i].vidx[global_param.crop_class[j]];
            printf("crop local: %zu, cell: %zu, blaat: %d\n", j, i, ivar_local_veg[i]);
        }
        printf("hoi: %d\n", mpi_rank);
    ...
    if (mpi_rank == VIC_MPI_ROOT) {
        for (j = 0; j < global_param.nr_crops; j++) {
            for (i = 0; i < global_domain.ncells_active; i++) {
                printf("crop global: %zu, cell: %zu, blaat: %d\n", j, i, ivar_global_veg[j][i]);
            }
        }
        /*******************************
        ...
        for (i = 0; i < global_domain.ncells_active; i++) {
            ...
            for (j = 0; j < global_param.nr_crops; j++) {
                ...
                if (ivar_local_veg[i] != NODATA_VEG) {
                    RID.nr_irr_cells++;
                    break;
                }
            }
        ...
        for (i = 0; i < global_domain.ncells_active; i++) {
            ...
            printf("cell %zu: nr_crops %zu\n", i, RID.cells[i].irr->nr_crops);
        }
        ...
        for (i = 0; i < global_domain.ncells_active; i++) {
            for (j = 0; j < global_param.nr_crops; j++) {
                if (ivar_global_veg[j][i] != NODATA_VEG) {
                    ...
                    RID.irr_cells[i].veg_class[iCrop] = global_param.crop_class[j];
...
void set_irr_crops() {
    ...
    for (i = 0; i < RID.nr_irr_cells; i++) {
         ...
         *******************************/
        for (j = 0; j < global_param.nr_crops; j++) {
            if (veg_con_map[RID.irr_cells[i].cell->id].vidx[global_param.crop_class[j]] != NODATA_VEG) {
        ...
        for (j = 0; j < global_param.nr_crops; j++) {
            if (veg_con_map[RID.irr_cells[i].cell->id].vidx[global_param.crop_class[j]] != NODATA_VEG) {
                RID.irr_cells[i].crop_index[iCrop] = j;
                RID.irr_cells[i].veg_class[iCrop] = global_param.crop_class[j];
                RID.irr_cells[i].veg_index[iCrop] = veg_con_map[RID.irr_cells[i].cell->id].vidx[global_param.crop_class[j]];
                iCrop++;
            }
        }
        ...
        for (j = 0; j < RID.irr_cells[i].nr_crops; j++) {
            ...
            soil_con[RID.irr_cells[i].cell->id].Ksat[0] = global_param.crop_ksat;
        }
