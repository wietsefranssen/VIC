...
ext_run(dmy_struct dmy)
{
    ...
    if(mpi_decomposition == BASIN_DECOMPOSITION){
        for(i=0;i<local_domain.ncells_active;i++){
            ...
            if(ext_options.DAMS){
                for(j=0;j<dam_con_map[cur_id].Ndams;j++){
                    dam_run(dam_con[cur_id][j],
                            &ext_all_vars[cur_id].dam_var[j], 
                            &ext_all_vars[cur_id].rout_var,
                            veg_con[cur_id],
                            veg_con_map[cur_id],
                            local_domain.locations[cur_id],
                            dmy);
                }                
            }
        }
        ...
        log_err("MPI decomposition not implemented");
    }
    timer_stop(&timer);
