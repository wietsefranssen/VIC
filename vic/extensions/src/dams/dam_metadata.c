...
{    
    ...
    strcpy(out_metadata[OUT_DAM_VOLUME].standard_name,
           "dam_water_volume");
    strcpy(out_metadata[OUT_DAM_VOLUME].units, "km3");
    ...
    strcpy(out_metadata[OUT_DAM_AREA].long_name, "dam_water_area");
    strcpy(out_metadata[OUT_DAM_AREA].standard_name,
           "dam_water_area");
    strcpy(out_metadata[OUT_DAM_AREA].units, "km2");
    strcpy(out_metadata[OUT_DAM_AREA].description,
           "dam_water_area");

    strcpy(out_metadata[OUT_DAM_HEIGHT].varname, "OUT_DAM_HEIGHT");
    strcpy(out_metadata[OUT_DAM_HEIGHT].long_name, "dam_water_height");
    ...
    out_metadata[OUT_DAM_VOLUME].nelem = ext_options.MAXDAMS;
    out_metadata[OUT_DAM_DISCHARGE].nelem = ext_options.MAXDAMS;
    out_metadata[OUT_DAM_AREA].nelem = ext_options.MAXDAMS;
    out_metadata[OUT_DAM_HEIGHT].nelem = ext_options.MAXDAMS;
    out_metadata[OUT_DAM_OP_DISCHARGE].nelem = ext_options.MAXDAMS;
    out_metadata[OUT_DAM_OP_VOLUME].nelem = ext_options.MAXDAMS;
    out_metadata[OUT_DAM_OP_MONTH].nelem = ext_options.MAXDAMS;
}
