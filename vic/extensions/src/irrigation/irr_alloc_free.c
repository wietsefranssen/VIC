...
irr_set_mapping(void)
{
    ...
    status = MPI_Bcast(&ivar, ext_options.NIRRTYPES, MPI_UNSIGNED_LONG, VIC_MPI_ROOT, MPI_COMM_VIC);
...
irr_set_nseasons(void)
{
    ...
    for(j = 0; j < (size_t)ext_options.NIRRTYPES; j++){
        ...
        get_scatter_nc_field_int(&(ext_filenames.irrigation), 
                ext_filenames.info.nseason, d3start, d3count, ivar);
...
irr_alloc(void)
{
    ...
    for(i=0; i<local_domain.ncells_active; i++){  
        irr_con_map[i].ni_types = ext_options.NIRRTYPES;
