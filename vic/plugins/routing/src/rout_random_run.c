...
rout_random_run() {
    ...
    //    }
    ////    log_info("\n ###2 mpi_rank: %d, stream_global[qq] %.4f", stream_global[qq], mpi_rank);
    //    log_info("\n ###2 mpi_rank: %d, dis_local[qq] %.4f", dis_local[qq], mpi_rank);

    // Calculate
    //   if (mpi_rank == VIC_MPI_ROOT) {
    for (i = 0; i < global_domain.ncells_active; i++) {
        if (dis_global[i] >= 1) log_info("###1 mpi_rank: %d, dis_global[%zu] %.4f", mpi_rank, i, dis_global[i]);
    }
    for (i = 0; i < local_domain.ncells_active; i++) {
        if (dis_local[i] >= 1) log_info("###1 mpi_rank: %d, dis_local[%zu] %.4f", mpi_rank, i, dis_local[i]);
    }
    ////            
    ...
    // if (mpi_rank == VIC_MPI_ROOT) {
    for (i = 0; i < global_domain.ncells_active; i++) {
        if (dis_global[i] >= 1) log_info("###3 mpi_rank: %d, dis_global[%zu] %.4f", mpi_rank, i, dis_global[i]);
    }
    for (i = 0; i < local_domain.ncells_active; i++) {
        if (dis_local[i] >= 1) log_info("###3 mpi_rank: %d, dis_local[%zu] %.4f", mpi_rank, i, dis_local[i]);
    }
    //            cur_cell = routing_order[i];
    //            log_info("### i: %zu, cur_cell %zu", i,cur_cell);

    //    log_info("\n ###3 mpi_rank: %d, stream_local[qq] %.4f", stream_local[qq], mpi_rank);
