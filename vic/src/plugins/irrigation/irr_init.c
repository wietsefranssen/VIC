    ...
    for (i = 0; i < local_domain.ncells_active; i++) {
        for (j = 0; j < irr_con_map[i].ni_types; j++) {
            if (irr_con_map[i].iidx[j] != NODATA_VEG) {
                irr_con[i][irr_con_map[i].iidx[j]].veg_index =
                    irr_con_map[i].vidx[j];
            }
        }
    }
}
        ...
        for (j = 0; j < (size_t)options.NIRRTYPES; j++) {
            ...
            for (i = 0; i < local_domain.ncells_active; i++) {
                if (irr_con_map[i].iidx[j] != NODATA_VEG &&
                    ...
                    irr_con[i][irr_con_map[i].iidx[j]].season_start[k] =
                        dvar[i];
                }
            }
            ...
            for (k = 0; k < irr_con[i][j].nseasons; k++) {
                for (l = 0; l < irr_con[i][j].nseasons; l++) {
                    if (k != l &&
                        ...
                        log_info(
                            "Cell %zu; crop %zu [veg_class %d]; season %zu [%.2f - %.2f] and %zu [%.2f - %.2f]",
                            i, j,
                            veg_con[i][irr_con[i][j].veg_index].veg_class,
                            k, irr_con[i][j].season_start[k],
                            irr_con[i][j].season_end[k],
                            l, irr_con[i][j].season_start[l],
                            irr_con[i][j].season_end[l]);
