        ...
        for (j = 0; j < irr_con_map[i].ni_types; j++) {
            ...
            if (cur_veg != NODATA_VEG) {
                if (nirr >= irr_con_map[i].ni_active) {
                    log_err(
                        "Number of irrigated vegetation classes does not match vegetation classes");
                }
...
irr_alloc(void)
{
    ...
    for (i = 0; i < local_domain.ncells_active; i++) {
        ...
        irr_con_map[i].vidx =
            malloc(irr_con_map[i].ni_types * sizeof(*irr_con_map[i].vidx));
        check_alloc_status(irr_con_map[i].vidx, "Memory allocation error");
        irr_con_map[i].iidx =
            malloc(irr_con_map[i].ni_types * sizeof(*irr_con_map[i].iidx));
    ...
    for (i = 0; i < local_domain.ncells_active; i++) {
        for (j = 0; j < irr_con_map[i].ni_active; j++) {
            irr_con[i][j].season_start =
                malloc(irr_con[i][j].nseasons *
                       sizeof(*irr_con[i][j].season_start));
            check_alloc_status(irr_con[i][j].season_start,
                               "Memory allocation error");
            irr_con[i][j].season_end =
                malloc(irr_con[i][j].nseasons *
                       sizeof(*irr_con[i][j].season_end));
            check_alloc_status(irr_con[i][j].season_end,
                               "Memory allocation error");
        }
    }
