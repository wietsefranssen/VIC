...
routing_rvic_init(void)
{
    ...
    if (mpi_rank == VIC_MPI_ROOT) {
        ...
        d3start[1] = 0;
        d3start[2] = 0;
        d3count[0] = routing_rvic.rout_param.n_timesteps;
        d3count[1] = routing_rvic.rout_param.n_sources;
        d3count[2] = 1; // tracers dimension

        // allocate memory for variables to be read
        ivar = malloc(routing_rvic.rout_param.n_sources * sizeof(*ivar));
        ...
        for (j = 0; j < global_domain.ncells_active; j++) {
            routing_rvic.discharge[j] = 0.0;
        }

        // source2outlet_ind: source to outlet index mapping
        get_nc_field_int(&(filenames.rout_params),
                         "source2outlet_ind",
                         &i1start, &routing_rvic.rout_param.n_sources, ivar);
        for (i = 0; i < routing_rvic.rout_param.n_sources; i++) {
            routing_rvic.rout_param.source2outlet_ind[i] = (int) ivar[i];
        }
        ...
        get_nc_field_int(&(filenames.rout_params),
                         ...
                         &i1start, &routing_rvic.rout_param.n_sources, ivar);
        for (i = 0; i < routing_rvic.rout_param.n_sources; i++) {
            routing_rvic.rout_param.source_time_offset[i] = (int) ivar[i];
        }
        ...
        get_nc_field_int(&(filenames.rout_params),
                         ...
                         &i1start, &routing_rvic.rout_param.n_sources, ivar);
        for (i = 0; i < routing_rvic.rout_param.n_sources; i++) {
            routing_rvic.rout_param.source_x_ind[i] = (int) ivar[i];
        }
        ...
        get_nc_field_int(&(filenames.rout_params),
                         ...
                         &i1start, &routing_rvic.rout_param.n_sources, ivar);
        for (i = 0; i < routing_rvic.rout_param.n_sources; i++) {
            routing_rvic.rout_param.source_y_ind[i] = (int) ivar[i];
        }

        // source_lat: Latitude coordinate of source grid cell
        get_nc_field_double(&(filenames.rout_params),
                            "source_lat",
                            &i1start, &routing_rvic.rout_param.n_sources, dvar);
        for (i = 0; i < routing_rvic.rout_param.n_sources; i++) {
            routing_rvic.rout_param.source_lat[i] = (double) dvar[i];
        }
        ...
        get_nc_field_double(&(filenames.rout_params),
                            ...
                            &i1start, &routing_rvic.rout_param.n_sources, dvar);
        for (i = 0; i < routing_rvic.rout_param.n_sources; i++) {
            routing_rvic.rout_param.source_lon[i] = (double) dvar[i];
        }

        // outlet_lat: Latitude coordinate of source grid cell
        get_nc_field_double(&(filenames.rout_params),
                            "outlet_lat",
                            &i1start, &routing_rvic.rout_param.n_outlets, dvar);
        for (i = 0; i < routing_rvic.rout_param.n_outlets; i++) {
            routing_rvic.rout_param.outlet_lat[i] = (double) dvar[i];
        }
        ...
        get_nc_field_double(&(filenames.rout_params),
                            ...
                            &i1start, &routing_rvic.rout_param.n_outlets, dvar);
        for (i = 0; i < routing_rvic.rout_param.n_outlets; i++) {
            routing_rvic.rout_param.outlet_lon[i] = (double) dvar[i];
        }
        ...
        for (i_source = 0;
            ...
            for (i = 0; i < global_domain.ncells_total; i++) {
                if (routing_rvic.rout_param.source_lat[i_source] ==
                    ...
                    routing_rvic.rout_param.source_lon[i_source] ==
                    global_domain.locations[i].longitude) {
                    routing_rvic.rout_param.source_VIC_index[i_source] = i;
                }
            }
        }
        ...
        for (i_outlet = 0;
            ...
            for (i = 0; i < global_domain.ncells_total; i++) {
                if (routing_rvic.rout_param.outlet_lat[i_outlet] ==
                    global_domain.locations[i].latitude &&
                    routing_rvic.rout_param.outlet_lon[i_outlet] ==
                    global_domain.locations[i].longitude) {
                    routing_rvic.rout_param.outlet_VIC_index[i_outlet] = i;
                }
            }
        }
