...
rout_alloc(void)
{
    ...
    for(i=0; i<local_domain.ncells_active; i++){        
        ...
        rout_con[i].grid_irf = malloc(options.GIRF_NSTEPS * sizeof(*rout_con[i].grid_irf));
        check_alloc_status(rout_con[i].grid_irf,"Memory allocation error");
        
        rout_var[i].discharge = malloc(options.RIRF_NSTEPS * sizeof(*rout_var[i].discharge));
        check_alloc_status(rout_var[i].discharge,"Memory allocation error");
        
        rout_var[i].nat_discharge = malloc(options.RIRF_NSTEPS * sizeof(*rout_var[i].nat_discharge));
